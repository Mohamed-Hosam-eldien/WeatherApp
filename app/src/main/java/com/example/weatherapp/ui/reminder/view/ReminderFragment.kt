...
class ReminderFragment : Fragment(), DatePickerDialog.OnDateSetListener,
    ...
    private fun methoud() {
        ...
        alarmIntent = Intent(context, BroadcastNoti::class.java).let { intent ->
...
// 20 minutes.
    ...
    private fun scheduleNotification(notification: Notification, delay: Long) {
        val notificationIntent = Intent(requireContext(), BroadcastNoti::class.java)
        notificationIntent.putExtra(BroadcastNoti.NOTIFICATION_ID, 1)
        notificationIntent.putExtra(BroadcastNoti.NOTIFICATION, notification)
